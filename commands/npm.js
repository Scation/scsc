...
if (args.length === 0) return message.reply('**/ Vous devez fournir un terme de recherche. \**');
...
.addField(`Description`, `${version.description || 'Pas de description.'}\n\u200B`)
.addField('Dépendances', `${deps && deps.length ? deps.join(', ') : '*None*'}\n\u200B`, false)
.addField('Dernière modification', `${updated} ago`, true)
.addField('Version', `${body['dist-tags'].latest}`, true)
.addField('License', `${body.license}\n\u200B`, true)
.addField('Mainteneurs', maintainers.join(', '), true)
...
if(error.status == 404) return message.channel.send('**/ Je ne trouve pas de résultat. \**');
console.log(error);
}  
}
module.exports.help = {
